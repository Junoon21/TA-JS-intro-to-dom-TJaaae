...
//    - Select the first `li` element inside the `ul` with class `topics` and change the innerText to `all about d`
document.body.children[5].children[0].innerText = "all about d";
//    - Select the inp(ut element with name `email`
...
let total = document.querySelectorAll("img");
console.log(`The total number of img element is ---${total.length}`);
...
let allLabelElements = document.querySelectorAll("label");
console.log(allLabelElements.length);
// 19. Select all the elements with `id` of `test`
let allIdElements = document.getElementById("test");
...
topics.previousElementSibling.innerText = "walking the dom";
